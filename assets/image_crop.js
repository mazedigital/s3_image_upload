...
$(document).ready(function(){
    ...
    function resizeInCanvas(){
	    ...
	    jQuery('.image-preview-container img').attr('src',base64img);
    }
    ...
    $(document).mouseup(function(e) {
        ...
        jQuery('input[name="'+fieldPrefix+'[top]"]').val($frame.data('top'));
        jQuery('input[name="'+fieldPrefix+'[left]"]').val($frame.data('left'));
        jQuery('input[name="'+fieldPrefix+'[width]"]').val($frame.data('width'));
        jQuery('input[name="'+fieldPrefix+'[height]"]').val($frame.data('height'));
    ...
    if ($('#source-img').length > 0){
    	...
    	img.onload = function(e) {
			...
			$frame.css('top',(jQuery('input[name="'+fieldPrefix+'[top]"]').val() * imgHeight / 100) + 'px');
			$frame.css('height',(jQuery('input[name="'+fieldPrefix+'[height]"]').val() * imgHeight / 100) + 'px');
			$frame.css('left',(jQuery('input[name="'+fieldPrefix+'[left]"]').val() * imgWidth / 100) + 'px');
			$frame.css('width',(jQuery('input[name="'+fieldPrefix+'[width]"]').val() * imgWidth / 100) + 'px');
    	};
    	var $img = $(img);
	...
	var selected = jQuery('input[name="'+fieldPrefix+'[crop_position]').val();
	jQuery(' .col[data-pos="'+ selected +'"]').addClass('active');

	jQuery(document).on('click','.grid .col',function(){
		// alert(jQuery(this).data('pos'));
		jQuery('.col').removeClass('active');
		jQuery(this).addClass('active');
		var selected = jQuery(this).data('pos');

		jQuery('.image-preview-container img').removeClass().addClass(selected)
		jQuery('input[name="'+fieldPrefix+'[crop_position]').val(selected);
	});
	...
	myDropzone.on("thumbnail", function(file) {
		var height = jQuery('.image-wrap img').height();
		var width = jQuery('.image-wrap img').width();
		jQuery('.grid').height(height);
		jQuery('.grid').width(width);

        var fieldPrefix = $('.field-imagecropper').data('field-name');
        jQuery('input[name="'+fieldPrefix+'[image]"]').val($('#source-img').attr('src'));
        jQuery('input[name="'+fieldPrefix+'[imagename]"]').val(file.name);
	});
